...
Current issues:s
- Unable to use "/udi" in-game commands (fixed 1.0.4)
...
1.0.4 Finally fixed "/udi" commands. Can now be toggled off or on ingame for debugging purposes. Unfortunately due to how item ownership is handled, UUID is still stored meaning if a claimed item is removed with the mod disabled, it will return again as soon the mod is enabled.
